...
class LISA():
    def __init__(self,m=1.328,lam=905,rmax=200,rmin=3,bdiv=1e-3,dst=0.05,
    ...
    def augment_mc(self,pc,Rr):
        ...
        for i in range(leng):
            ...
            ref  = pc[i,3]
            if ref!=0:
                pc_new[i,:]  = self.lisa_mc(x,y,z,ref,Rr,alpha)
        return pc_new
    
    def lisa_mc(self,x,y,z,ref,Rr, alpha):
        '''
        ...
        labl : 0, 1, 2
        '''
        #self.rmax = np.log(1.4) / (2*alpha) #lower 1.5 to make more aggressive -> low rmax, high P min, many points lost at a particular Rr or alpha
        rmax = self.rmax                      # max range (m)
        Pmin = 0.9*rmax**(-2) / np.pi      # min measurable power (arb units) / np.pi
        ...
        ran_r = ran*(np.random.rand(Nt))**(1/3) # sample Nt many distances from a quadratic pdf
        indx  = np.where(ran_r>rmin)[0]         # keep points where ranges larger than rmin
        Nt    = len(indx)                       # new particle number
        
        P0  = ref*np.exp(-2*alpha*ran)/(ran**2) # power #TODO: remove ran**2 bcz reflectivity already has r2 in it
    ...
    def augment_avg(self,pc):
        #TODO: Add random scattering if Pdropletmax is greater than P0
        ...
        ref_new[indp]    = ref[indp]*np.exp(-2*alpha*ran[indp])                   # new reflectance modified by attenuation/scattering #TODO there is no backscattering coeff, keep ref_new = 0 if P<Pmin
    ...
    def haze_point_cloud(self,pts_3D,Rr=0):
        ...
        pts_3D_detectable = pts_3D[detectable_points]
        shp = pts_3D_detectable.shape  # data shape
        pc_new = np.zeros((shp[0], shp[1] + 1))  # init new point cloud
        pc_new[:, :4] = pts_3D_detectable
        leng = shp[0]  # data length
        ...
        dmax = -np.divide(np.log(np.divide(n,(pts_3D_detectable[:,3] + g))),(2 * beta_usefull))
        dnew = -np.log(1 - 0.5) / (beta_usefull) # d of scatter points
        ...
        pc_new[cloud_scatter_idx, :] = cloud_scatter
        ...
        random_scatter[:,0:4] = pts_3D_detectable[random_scatter_idx,:]
        ...
        random_scatter[:, 4] = 3*np.ones(np.shape(random_scatter[:, 3]))
        pc_new[random_scatter_idx,:] = random_scatter

    
        dist_pts_3d = np.concatenate((old_points, cloud_scatter,random_scatter), axis=0)
    
        return pc_new #dist_pts_3d
    ...
    def calc_Mie_params(self):
        ...
        out   = ps.MieQ_withDiameterRange(self.m, self.lam, diameterRange=(1,1e7),
                                        nd=2000, logD=True) #2000 bins, diameter range is in nm (1nm, 10mm or 1cm)
        D     = out[0]*1e-6 # convert diameter bins to mm
        qext  = out[1]
        qback = out[6]
    ...
    # SNOW #TODO SEkhon et al distribution model missing
    def N_MG_snow(self,D,Rr):
        '''
        Marshall - Gunn snow model (Gunn and Marshall 1957)
    ...
    def MG_Sample_snow(self,Rr,N,dstart):
        '''
        Sample particle diameters from Marshall Gunn distribution
        ...
        diameters = -np.log(1-r)/lmda + dstart
        return diameters
    # FOG #TODO: add rain model as well, add sampling and backscattering to fog
    ...
    # D is in mm, we convert it to um (i.e. x 10^3) to give it to N_GD
    def Nd_haze_coast(self,D):
        return 1e9*self.N_GD(D*1e3,rho=100,alpha=1,g=0.5,Rc=0.05)
    ...
    # Rain coast
    def Nd_rain_coast(self, D):
        return 1e9 * self.N_GD(D * 1e3, rho=1000*1e-6, alpha=1, g=0.5, Rc=0.05 * 1e3)

    # Rain continental
    def Nd_rain_continental(self, D):
        return 1e9 * self.N_GD(D * 1e3, rho=1000 * 1e-6, alpha=2, g=0.5, Rc=0.07 * 1e3)
